...
def detector(cap: object):
    ...
    while cap.isOpened():
        ...
        for box in bboxes:
            ...
            cv2.circle(img=frame, center=(xc, yc), radius=8, color=(0,255,0), thickness=-1)
            cv2.rectangle(img=frame, pt1=(box[0], box[1]), pt2=(box[2], box[3]), color=(255, 0, 0), thickness=1)

        cv2.putText(img=frame, text=f"Carros: {detections}", org=(100, 100), fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=3, color=(0,0,255), thickness=3)
...
if __name__ == '__main__':
    ...
    cap = cv2.VideoCapture("data/traficobogota3.mp4")
